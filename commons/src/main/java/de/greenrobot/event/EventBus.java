...
public class EventBus {
    ...
    void invokeSubscriber(Subscription subscription, Object event) {
        try {
            //FIXME: ojrl: validation of broadcast vs p2p
            boolean check1 = checkP2PSubscription( subscription.subscriber, event );
            boolean check2 = checkExceptions( subscription.subscriber, event  );
    ...
    //FIXME: ojrl: validation of broadcast vs p2p
    /**
     * this method returns true when this is either a broadcast message or a p2p message for the
     * corresponding receiver, otherwise false
     * @param subscriber
     * @param event
     * @return
     */
    private boolean checkP2PSubscription( Object subscriber, Object event ) {
        ...
        try {
            Method method = clazz.getMethod( "getSubscriber", (Class<?>)null );
            if( method != null ) {
                Object subscriberEv = method.invoke(event);
                if (subscriberEv != null && subscriberEv != subscriber) {
                    return false;
                }
            }
        } catch (Exception e) {
