package com.yahoo.inmind.control.yi13n;
/**
 * Borrowed from YI13N
 * */
...
public class LocationTracker {
		...
		if(!trackingRunning){
			new Thread(){
				...
				public void run(){
					...
					while(runThread){
						locationManager = (LocationManager) ReaderController.get().getSystemService(Context.LOCATION_SERVICE);
  ...
  @TargetApi(9)
	...
	private static boolean hasNetworkPermission(){
	    int res = ReaderController.get().checkCallingOrSelfPermission("android.permission.ACCESS_COARSE_LOCATION");
	...
	private static boolean hasGPSPermission(){	
	    int res = ReaderController.get().checkCallingOrSelfPermission("android.permission.ACCESS_FINE_LOCATION");
