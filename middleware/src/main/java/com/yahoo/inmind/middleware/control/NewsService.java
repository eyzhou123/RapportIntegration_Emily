...
import com.yahoo.inmind.middleware.events.news.FilterByEmailEvent;
...
//        getNewsItems( request );
        ...
        try {
            ...
            if (request.get(Constants.BUNDLE_TYPE_FILTER) != null) {
                if (request.get(Constants.BUNDLE_TYPE_FILTER).equals("AND")) {
                    reader.typeOfFilter = "AND";
                } else {
                    reader.typeOfFilter = "OR";
                }
            }
    ...
    public void onEventAsync(FilterByEmailEvent event){
        ...
        for( String category : categories ){
            filters.add( new FilterVO( Constants.ARTICLE_CATEGORIES,
                    Constants.FILTER_EQUALS_TO, category ) );
        }
    ...
    public ModelDistribution getModelDistributions( MBRequest mbRequest ){
        return reader.getModelDistributions();
    }
